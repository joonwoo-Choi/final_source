package com.bh.view
{
	...
	public class CardSection
	{
		...
		private function defaultTxtMask():void
		{
			var tm:TextFormat = new TextFormat();
			tm.letterSpacing = -20;
			tm.size = 160;
			_con.maskTxt.txt.embedFonts = true;
			_con.maskTxt.txt.defaultTextFormat = tm;
			_con.maskTxt.txt.y = 550;
			_maskTxtYpos = _con.maskTxt.txt.y;
		}
		
		private function closeUpTxtMask():void
		{
			var tm:TextFormat = new TextFormat();
			tm.letterSpacing = -20;
			tm.size = 200;
...
//			var reflection:Boolean = Boolean(Math.round(Math.random()));
			_colLength = Math.ceil(_con.maskTxt.txt.width/(_cardWidth));
...
//			else sp.rotationZ = Math.random()*45;
		...
		private function playCard():void
		{
			...
			for (i = 0; i < _totalLength; i++) 
			{
				...
				{
					delayNum = i%2;
					TweenLite.to(_cardArr[i], 0.5, {delay:Math.random()*0.5, 
					alpha:1, onComplete:intervalMotion, onCompleteParams:[_cardArr[i]]});
				}
			}
...
//					TweenLite.to(_cardArr[i], 0.35, {rotationX:90, alpha:0, onComplete:hideCompleteCheck});
					TweenLite.to(_cardArr[i], 0.5, {delay:Math.random()*0.5, alpha:Math.random()*0.6+0.2, onComplete:hideCompleteCheck});
				}
			}
		}
