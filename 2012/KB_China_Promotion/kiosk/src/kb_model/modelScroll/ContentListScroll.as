package kb_model.modelScroll
{
	...
	public class ContentListScroll extends Sprite
	{
		...
		public function setting():void
		{
			...
			$model.addEventListener(ModelEvent.GO_TO_MODEL_MAIN, gotoMain);
		...
		protected function gotoMain(event:Event):void
		{
			reset();
		}
		...
		private function removePrevMC():void
		{
			var num:int = $con.numChildren;
			for (var i:int = 0; i < num; i++) 
			{
...
//			if( $allList[0].numChildren > 1) $allList[0].removeChildAt(1) as Object;
		...
		/**	리스트 이동 방향 설정	*/
		public function directionChange(moveY:Number):void
		{
			if(moveY >=10 )
			{
				for (var i:int = 0; i < $allListLength; i++) 
		...
		{
			// 						썸네일 전체 배열개수
			trace("$currentListLength: ",$currentListLength);
			for (var i:int = 0; i < $currentListLength; i++) 
			{
				if(i == listNum)
				{
