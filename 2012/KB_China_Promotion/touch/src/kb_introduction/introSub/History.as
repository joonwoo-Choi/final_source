package kb_introduction.introSub
{
	...
	public class History extends Sprite
	{
		...
		private function xmlLoad():void
		{
			...
			ldr.load(new URLRequest($model.defaultURL + "xml/introductionHistoryList.xml"));
		...
		private function imgLoadComplete(e:Event):void
		{
			...
			for (var i:int = 0; i < $listLength; i++) 
			{
				...
				$listArr[i].x = $main.maskMC.x + $main.maskMC.width/2;
			}
		...
		private function mouseUpHandler(e:MouseEvent):void
		{
			...
			{
				if($listArr[$listNum].width > $main.listCon.scroll.x)
				{
					TweenLite.to($listArr[$listNum], 0.75, {x:$main.listCon.scroll.x - $listArr[$listNum].width, alpha:1, ease:Expo.easeOut});
				}
				else
				{
		...
		private function listChange():void
		{
			for (var i:int = 0; i < $listLength; i++) 
			{
				...
				if($listNum == i) 
				{
					...
					/**	컨텐츠 스크롤 보이기 숨기기	*/
					if($listArr[i].imgCon.height > 414)
