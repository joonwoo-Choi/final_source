...
//			if($model.popupNum > 0)
//			{
//				TweenMax.to($con, 0.75, {autoAlpha:1, ease:Cubic.easeOut});
//			}
			...
			for (var i:int = 1; i < $listLength; i++) 
			{
				if($model.popupNum == i)
				{
					TweenLite.to($listArr[i], 0.75, {alpha:1, ease:Cubic.easeOut});
				}
				else
				{
					$listArr[i].alpha = 0;
//					TweenLite.to($listArr[i], 0.75, {alpha:0, ease:Cubic.easeOut});
				}
			}
