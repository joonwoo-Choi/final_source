package orpheus.templete.moviePlayer.ctrler
{
	...
	public class CtrlerBtn extends CtrlerMoviePlayer
	{
...
//				video_mc.thumbImg.btn.addEventListener( Event.ENTER_FRAME, fade );
		...
		private var _firstPlayChk:Boolean=false;
...
//			video_mc.thumbImg.addEventListener( Event.ENTER_FRAME, fade );
			var num:int = video_mc.thumbImg.numChildren;
			for (var i:int = 0; i < num; i++) 
			{
				var mc:DisplayObject = video_mc.thumbImg.getChildAt(i);
				TweenLite.to(mc,.5,{autoAlpha:0});
			}
			...
			if (video_mc.vid.btn[event.target.name] || event.target.name == "stgBg") {
				...
				if ((event.target.name == "ply" || event.target.name == "stgBg") ) {
					...
					if (!_model.autoPlayVideo) {
						...
						video_mc.thumbImg.btn.gotoAndStop(1);
						enableButtons();
		...
		private function ctrlVideo():void
		{
			// TODO Auto Generated method stub
			if(video_mc.thumbImg.btn.currentFrame==1){
				trace("ccc");
				video_mc.thumbImg.btn.gotoAndStop(2);
				_moviePlayer.localVideoMCControl.NS.pause();
			}else if(video_mc.thumbImg.btn.currentFrame==2){
				if(_firstPlayChk){
					trace("aaa");
					video_mc.thumbImg.btn.gotoAndStop(1);
					_moviePlayer.localVideoMCControl.NS.resume();
				...
				video_mc.thumbImg.btn.gotoAndStop(1);
			}else if(video_mc.thumbImg.btn.currentFrame==3){
				trace("ddd");
				video_mc.thumbImg.btn.gotoAndStop(1);
				video_mc.btns.ply.ico.gotoAndStop(1);
				loadNewVideo(_model.vidSource);
