package facebook.lunching
{
	 ...
	 */
	public class FriendInviteList extends BaseList
	{
		private var $xml:XML;
		/**	그래픽	*/
		...
		/**	생성자	*/
		public function FriendInviteList($scope:Object,$obj:Object)
		{
			selectCnt = 0;
			selectMax = 5;
			...
			if(ExternalInterface.available)ExternalInterface.addCallback("launchFlashCB",teaserFlash);
			init();
			if(!SecurityUtil.isWeb())load();
			
		}
		
		private function teaserFlash(fbAtoken:String):void
		{
...
//			if( ExternalInterface.available ) ExternalInterface.call( "alert" , "토큰값: "+fbAtoken );
		...
		override protected function onNoneList():void
		{
			...
			container.parent["mcLoadingFriend"].visible =false;
			trace("container.parent[mcLoadingFriend].visible: ",container.parent["mcLoadingFriend"].visible);
