package facebook.lunching
{
	...
	public class FriendInviteMain
	{
		...
		public function FriendInviteMain(mc:MovieClip,thumbClass:Class,type:String="W")
		{
			...
			if(ExternalInterface.available)ExternalInterface.addCallback("launchFlashLoginCB",launchFlashLoginCB);
		}
...
//			}
			else
			{
				...
				$friends=String(friendInfo);
				container.loading.alpha = 0;
				container.loading.visible = true;
				container.loading.gotoAndPlay(2);
				if(ExternalInterface.available)ExternalInterface.call("joinEvt");
			}
		}
		
		private function launchFlashLoginCB(result:String):void
		{
			// TODO Auto Generated method stub
			trace("result: ",result);
			if(result=="true"){
				if(ExternalInterface.available)ExternalInterface.call("fbFeedImg",$friends);
				resetBtn();
			}
			container.loading.visible = false;
