...
Social.config = {};
/** 카카오 자바스크립트 앱 키  */
Social.config.kakaoApplicationKey = "12c30752133781ea4aca64c13107f1dd";
...
(function($){
    ...
    var shareMethods = {
        ...
        shareOnBand: function(linkUrl, description){
            var param = 'bandapp://create/post?text=' + encodeURIComponent(description) + "&route=" + linkUrl;
            if(navigator.userAgent.match(/android/i)){
                /** 안드로이드   */
                setTimeout(function(){
//                    "intent:bandapp://create/post?text= &route= #Intent;package=com.nhn.android.band;end;"
        ...
        shareOnLine: function(linkUrl, description){
            if(/iphone|ipad|ipod|android/gi.test(navigator.userAgent))
				window.location.href = "http://line.naver.jp/R/msg/text/?" + encodeURIComponent(description) + " " + encodeURIComponent(linkUrl);
        },
        /** 메일  */
        shareOnMail: function(email, title, description){
            var mailTitle = title.trim();
            var mailToLink = "mailto:" + encodeURIComponent(email);
            if(mailTitle == "") mailToLink = mailToLink + "?body=" + encodeURIComponent(description);
        ...
        shareOnCopyUrl: function(linkUrl, description){
            if(/trident|msie/gi.test(navigator.userAgent)){
	...
	var kakaoUserName = null;
	var kakaoProfileImage = null;
	...
	Social.kakao = {
        ...
        /** 카카오톡 공유 */
		shareOnTalk: function(linkUrl, title, linkText, thumbnail, btnTitle){
